-- Tournament Beta fast singles setup and compact score controls.
...
declare
  ...
  v_slot int := 0;
begin
  if not (public.is_admin() or public.has_commissioner_permission('golf')) then
    raise exception 'Golf organizer access required';
  end if;
