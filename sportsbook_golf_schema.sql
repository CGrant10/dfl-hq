...
-- Retire the old soft auto props without calling commissioner-protected RPCs.
do $$
declare r record; b record;
begin
  for r in select sm.id from public.sportsbook_markets sm
    where sm.source='lore' and sm.auto_key is not null and sm.status in ('open','locked')
      and sm.auto_key in ('dfl-rule-smoke','dfl-groupchat-fantasy','dfl-golf-trash','dfl-late-arrival','dfl-bad-purchase','dfl-argument-source','marvel-trailer','marvel-doom','marvel-rewatch','gaming-rage','gaming-rematch','gaming-excuse','golf-lost-ball','golf-mulligan','golf-club-blame','fantasy-draft-talk','fantasy-keeper-regret','fantasy-sleeper-hype')
  loop
    for b in select sb.* from public.sportsbook_bets sb where sb.market_id=r.id and sb.status='open' for update loop
      ...
      insert into public.sportsbook_ledger(member_id,amount,kind,note,market_id,bet_id) values(b.member_id,b.stake,'refund','Retired auto-market refund',r.id,b.id);
    end loop;
...
-- Drop first so Postgres cannot preserve an older compiled body/signature.
...
declare o record; t record; m record; s1 record; s2 record; new_market_id bigint; close_at timestamptz; k text; note text;
begin
  ...
  close_at:=case when o.event_date is not null then (o.event_date::timestamp+coalesce(o.event_time,time '09:00')) at time zone current_setting('TIMEZONE') else now()+interval '72 hours' end;
  ...
  if (select count(*) from public.golf_teams gt where gt.outing_id=o.id)>=2 then
    ...
    if not exists(select 1 from public.sportsbook_markets sm where sm.auto_key=k and sm.status in ('open','locked','settled')) then
      ...
      values(o.name||' — Who wins the damn thing?','Golf','lore','Real DFL teams. Neutral opening prices for now: Lore will talk shit, but it will not fake a handicap.','open',close_at,k) returning id into new_market_id;
      for t in select gt.id,gt.name,gt.sort_order from public.golf_teams gt where gt.outing_id=o.id order by gt.sort_order,gt.id loop
        ...
        values(new_market_id,coalesce(t.name,'Team'),case (select count(*) from public.golf_teams gt2 where gt2.outing_id=o.id) when 2 then -110 when 3 then 200 when 4 then 300 else 400 end,coalesce(t.sort_order,0));
      end loop;
    end if;
  end if;
  ...
  if (select count(*) from public.golf_teams gt where gt.outing_id=o.id)=2 then
    ...
    if not exists(select 1 from public.sportsbook_markets sm where sm.auto_key=k and sm.status in ('open','locked','settled')) then
      insert into public.sportsbook_markets(title,category,source,lore_note,status,closes_at,auto_key)
      values(coalesce(s1.name,'Team 1')||' vs '||coalesce(s2.name,'Team 2')||' — Tournament moneyline','Golf','lore','The computer refuses to pretend either side has earned favorite status yet. Pick your idiots.','open',close_at,k) returning id into new_market_id;
      insert into public.sportsbook_outcomes(market_id,label,odds_american,sort_order) values(new_market_id,coalesce(s1.name,'Team 1'),-110,0),(new_market_id,coalesce(s2.name,'Team 2'),-110,1);
    end if;
  end if;

  for m in select gm.id,gm.match_number,gm.round_id,gr.round_number,gr.name as round_name,gr.format from public.golf_matches gm join public.golf_rounds gr on gr.id=gm.round_id where gm.outing_id=o.id order by gr.round_number,gm.match_number loop
    select gs.*,gt.name team_name into s1 from public.golf_match_sides gs join public.golf_teams gt on gt.id=gs.team_id where gs.match_id=m.id and gs.slot=1;
    select gs.*,gt.name team_name into s2 from public.golf_match_sides gs join public.golf_teams gt on gt.id=gs.team_id where gs.match_id=m.id and gs.slot=2;
    if s1.id is null or s2.id is null then continue; end if;
    k:='golf:'||o.id||':match:'||m.id;
    if not exists(select 1 from public.sportsbook_markets sm where sm.auto_key=k and sm.status in ('open','locked','settled')) then
      note:=case when m.format='singles' then 'Singles. No teammate to blame, no place to hide. The house has removed all adult supervision.' else 'Pairs. Four golfers, two balls, and enough shared blame to keep every group chat alive until winter.' end;
      insert into public.sportsbook_markets(title,category,source,lore_note,status,closes_at,auto_key)
      values('Round '||m.round_number||' · Match '||m.match_number||' — '||coalesce(s1.team_name,'Side 1')||' vs '||coalesce(s2.team_name,'Side 2'),'Golf','lore',note,'open',close_at,k) returning id into new_market_id;
      insert into public.sportsbook_outcomes(market_id,label,odds_american,sort_order) values(new_market_id,coalesce(s1.team_name,'Side 1'),-110,0),(new_market_id,coalesce(s2.team_name,'Side 2'),-110,1);
    end if;
  end loop;
  return query select count(*)::int,o.id,o.name from public.sportsbook_markets sm where sm.category='Golf' and sm.auto_key like 'golf:'||o.id||':%' and sm.status='open';
end; $$;
...
begin
  ...
  update public.sportsbook_markets sm set status='locked' where sm.source='lore' and sm.auto_key is not null and sm.status='open' and sm.closes_at is not null and sm.closes_at<=now();
  ...
  for t in select x.* from public.sportsbook_auto_templates x where x.active=true and not exists(select 1 from public.sportsbook_markets sm where sm.auto_key=x.template_key and sm.status in ('open','locked')) and not exists(select 1 from public.sportsbook_markets sm where sm.auto_key=x.template_key and sm.created_at>now()-make_interval(days=>x.cooldown_days)) order by md5(x.template_key||current_date::text) limit need loop
    insert into public.sportsbook_markets(title,category,source,lore_note,status,closes_at,auto_key) values(t.title,t.category,'lore',t.lore_note,'open',now()+make_interval(hours=>t.duration_hours),t.template_key) returning id into new_market_id;
    n:=0; for item in select * from jsonb_array_elements(t.outcomes) loop insert into public.sportsbook_outcomes(market_id,label,odds_american,sort_order) values(new_market_id,item->>'label',(item->>'odds')::int,n); n:=n+1; end loop;
  end loop;
  select count(*) into current_open from public.sportsbook_markets sm where sm.source='lore' and sm.auto_key is not null and sm.status='open' and (sm.closes_at is null or sm.closes_at>now());
