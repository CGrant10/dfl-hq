...
if(memberList)memberList.addEventListener("click",async e=>{const btn=e.target.closest("button[data-member]");if(!btn)return;const members=await loadMembers();const member=members.find(m=>String(m.id)===btn.dataset.member);if(!member)return;selectMember(member);if(!savedTheme()&&member.favorite_team)saveTheme(member.favorite_team);paintName();welcome?.classList.add("hidden");await registerUser(member.display_name);toast(`Welcome, ${member.display_name}`);renderRoute()});
