...
async function openPicker({ cancellable = false } = {}) {
  ...
  let members = [];
  try { members = await loadMembers({ force: true }); }
  catch { members = []; }
  ...
  memberList.innerHTML = members.map((m) => `
    <button type="button" class="memberbtn ${String(m.id) === mine ? "on" : ""}" data-member="${m.id}">
      ...
      <span class="memberbtn-text"><strong>${esc(m.display_name)}</strong>
        ${m.team_name ? `<span class="muted tiny">${esc(m.team_name)}</span>` : ""}</span>
    </button>`).join("");
}
