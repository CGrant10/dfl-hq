...
function positionReport(team) {
  return `<section class="ta-report-section">${sectionHead("STARTING UNIT GRADES", "Position report", `<span class="ta-inline-note">Grades use starting slots only · depth shown separately</span>`)}<div class="ta-table-wrap"><table class="ta-table ta-position-table"><thead><tr><th>Pos</th><th>Grade</th><th>League</th><th>Starter score</th><th>Starting unit / depth</th></tr></thead><tbody>${ANALYZER_POSITIONS.map(position => {
  ...
  return {
    ...
    wire(view) {
      ...
      const draw = () => {
        ...
        body.innerHTML = `${reportHeader(team, data.teams.length, data.pool)}${positionReport(team)}${rosterReport(team, data.pool)}${comparison(team, opponent, data.teams)}${rankings(data.teams, team.id)}${tradeLab(team, data.teams, data.pool, playerId)}<p class="ta-method">Method: projected finish and starter grade use only the submitted legal offensive lineup (1 QB, 2 RB, 2 WR, 1 TE and 1 flex), with an optimized lineup used only when the submitted starters are incomplete. Depth receives its own grade. Overall roster grade blends starters (72%), depth (18%) and top-12 roster value (10%). Position needs are league-relative and only appear for a genuinely weak starting unit. Player forecasts favor current projections and pace-adjust prior production. Estimates are not guarantees.</p>`;
