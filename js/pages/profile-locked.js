...
*/
function gate(view,member){view.innerHTML=`<div id="profile-lock-gate"><div class="pin-panel"><span class="pin-mark">${icon("lock",{size:24})}</span><h1>Profile locked</h1><p class="muted">${esc(member.display_name)} keeps this one behind a PIN.</p><form id="profile-unlock-form" autocomplete="off"><label for="profile-unlock-pin">Profile PIN</label>${pinInput("profile-unlock-pin","dfl-profile-unlock").replace("<input ","<input class=\"pin-input\" ")}<div class="row-end"><a class="btn ghost" href="#/home">Back</a><button class="btn" type="submit">Unlock profile</button></div></form><p class="muted tiny pin-note">Forgot it? A DFL Owner can reset the lock, but nobody can read the PIN.</p></div></div>`;const form=view.querySelector("#profile-unlock-form"),input=view.querySelector("#profile-unlock-pin");input?.addEventListener("input",()=>{input.value=input.value.replace(/\D/g,"").slice(0,6)});setTimeout(()=>input?.focus(),0);form?.addEventListener("submit",async e=>{e.preventDefault();const btn=form.querySelector("button[type=submit]");btn.disabled=true;try{const{data,error}=await db().rpc("profile_verify_pin",{target_member_id:Number(member.id),attempted_pin:input.value});if(error)throw error;if(data!==true)throw new Error("Wrong profile PIN");markUnlocked(member.id,input.value);toast("Profile unlocked");render(view);}catch(err){toast(err.message||"Could not unlock profile",true);btn.disabled=false;input.select();}});}
...
export async function render(view){let member;try{member=await targetMember()}catch(err){view.innerHTML=`<h1>Profile</h1><div class="card"><div class="card-body">${esc(err.message||"Could not load member")}</div></div>`;return;}if(!member)return profile.render(view);let locked=false,migrationReady=true;try{locked=await lockStatus(member.id)}catch{migrationReady=false}if(locked&&!unlocked(member.id)){gate(view,member);return;}await profile.render(view);const me=currentMember(),isMe=me&&String(me.id)===String(member.id);if(!isMe)return;const wrap=view.querySelector("#profile-wrap");if(!wrap)return;const section=document.createElement("section");section.innerHTML=`<h2 class="section-title">Privacy</h2>${settingsCard(member,locked,migrationReady)}`;wrap.appendChild(section);if(migrationReady)wireSettings(view,member,locked);}
