...
export async function renderCommissionerPanel(root) {
  ...
  try {
    ...
    if (error) throw error; members = data || [];
  } catch (err) { root.innerHTML = `<div class="card note"><div class="card-body">${esc(err.message || "Could not load members")}</div></div>`; return; }
  if (!members.length) { root.innerHTML = `<div class="card"><div class="card-body">Add league members before assigning commissioner access.</div></div>`; return; }

  root.innerHTML = `
    <div class="section-head"><h2>Commissioner Access</h2></div>
    <div class="card note"><div class="card-body">Give a league member their own commissioner PIN and only the tools they should control. The Owner setting grants every permission and the ability to manage commissioner access.</div></div>
    <form class="card" id="commissioner-form">
      <label for="commissioner-member">League member</label>
      <select id="commissioner-member" required>${members.map((m) => `<option value="${esc(m.id)}">${esc(m.display_name || m.team_name || `Member ${m.id}`)}</option>`).join("")}</select>
      <label for="commissioner-pin">New / reset PIN</label>
      <input id="commissioner-pin" type="password" inputmode="numeric" pattern="[0-9]*" minlength="4" required autocomplete="new-password" placeholder="At least 4 digits">
      <p class="muted">Saving always sets this member's commissioner PIN to the value above.</p>
      ...
      <div class="row-end"><button type="button" class="btn danger ghost" id="commissioner-disable">Disable access</button><button type="submit" class="btn">Save access</button></div>
    </form>
  ...
  owner.addEventListener("change", syncOwner); syncOwner();
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (error) throw error; pin.value = ""; toast(owner.checked ? "Owner access saved" : "Commissioner access saved");
    } catch (err) { toast(err.message || "Could not save commissioner access", true); }
    finally { submit.disabled = false; }
  });

  root.querySelector("#commissioner-disable").addEventListener("click", async () => {
    const chosen = members.find((m) => String(m.id) === String(member.value)); const name = chosen?.display_name || chosen?.team_name || "this member";
    ...
    try {
      ...
      if (error) throw error; pin.value = ""; owner.checked = false; permissionInputs.forEach((box) => { box.checked = false; }); syncOwner(); toast("Commissioner access disabled");
  ...
  root.querySelector("#profile-pin-reset").addEventListener("click", async () => {
    ...
    try {
      ...
      if (error) throw error; toast("Profile PIN lock reset");
    } catch (err) { toast(/profile_owner_reset_pin|function/i.test(err.message || "") ? "Run profile_lock_schema.sql in Supabase first" : (err.message || "Could not reset Profile PIN"), true); }
  });
}
