...
function scorecardHole(card){const selected=Number(card.querySelector("[data-gqm-entry]")?.dataset.activeHole);if(selected>0)return (selected-1)%9+1;for(const input of card.querySelectorAll("input[data-team-score]")){if(!String(input.value||"").trim())return ((Number(input.dataset.hole)||1)-1)%9+1}return 1}
...
export function setupCourseGps(config){
  ...
  function mount(){const view=document.getElementById("view"),query=new URLSearchParams(location.hash.split("?")[1]||"");if(!view||!location.hash.startsWith("#/golf")){stop();return}const card=view.querySelector('.dfl-team-card[data-quick-active="true"]')||view.querySelector(".dfl-team-card:not([hidden])")||view.querySelector(".dfl-team-card"),quick=card?.matches("[data-quick-player-card]");if(!card||(!query.get("team")&&!quick)||!config.courseRe.test(courseText(view))){stop();return}attach(card)}
  function boot(){window.addEventListener("hashchange",()=>setTimeout(mount,0));window.addEventListener("dfl:quick-player-change",()=>setTimeout(mount,0));const view=document.getElementById("view");if(view)new MutationObserver(()=>location.hash.startsWith("#/golf")&&mount()).observe(view,{childList:true,subtree:true});mount()}
