/* =====================================================================
   ...
   ---------------------------------------------------------------------
   Sunday and Monday are intentionally LIVE states. A projected lead before
   Monday Night Football is useful trash talk; calling it final is simply
   wrong. The numbers below come from the same current Sleeper projection
   bundle as Home's cold open, so the card and the app cannot disagree.
...
export function buildAftermath({ lore, members = [], weekly, now = new Date() } = {}) {
  ...
  if ((day !== 0 && day !== 1) || !weekly?.teams?.length || !weekly.season || !weekly.week) return null;

  const projections = new Map(weekly.teams.map(team => [key(team.sleeper_user_id), num(team.projection)]));
  ...
  const pairs = current.map(row => {
    ...
    const left = projections.has(key(row.user1)) ? projections.get(key(row.user1)) : num(row.score1);
    const right = projections.has(key(row.user2)) ? projections.get(key(row.user2)) : num(row.score2);
  ...
  const ranked = [...weekly.teams].sort((a, b) => num(b.projection) - num(a.projection));
  const king = ranked[0];
  if (!king) return null;
  ...
  const label = day === 0 ? "SUNDAY AFTERMATH" : "MONDAY AFTERMATH";
  const status = day === 0 ? "SUNDAY SLATE · LIVE" : "PRE-MNF · LIVE";

  return {
    label, status, season: Number(weekly.season), week: Number(weekly.week),
    king: { name: king.team_name || teamName(members, king.sleeper_user_id), value: num(king.projection) },
...
export function aftermathCanvas(card) {
  ...
  caps(ctx, "WEEK'S PROJECTED KING", W / 2, 292, SHARE_INK.INK, 30);
  ctx.fillStyle = SHARE_INK.GOLD;
  ...
  award(ctx, {
    x: 70, y: 640, w: 440, label: "BIGGEST PROJECTED GAP", name: left.winner,
  ...
  award(ctx, {
    ...
    value: score(right.value), detail: card.pain ? "PROJECTED TO LOSE" : "WAITING ON SCORES", accent: SHARE_INK.CREST_BLUE,
  });
...
export function aftermathText(card) {
  return `${card.label}: ${card.king.name} leads the Week ${card.week} projection at ${score(card.king.value)}. ${card.status}.`;
}
