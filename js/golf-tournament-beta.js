...
function calmSetupMarkup(state) {
  ...
  const roundCard = format => {
    ...
    return `<details class="tb-fold" data-tb-section="${format}" ${openSection === format || (!openSection && entry && format === "pairs") ? "open" : ""}><summary><span><small>${format === "pairs" ? "ROUND 1" : "ROUND 2"}</small><strong>${format === "pairs" ? "Three 2v2 matchups" : "Six singles matchups"}</strong></span><em class="${ready ? "is-ready" : ""}">${entry ? `${scoringName} · ${entry.battles.length}/${betaMatchCount(format)} built` : "Not built"}</em></summary><div class="tb-fold-body"><p class="tb-copy">${explainer}</p>${entry ? `<div class="tb-format-state"><small>ROUND SCORING</small><strong>${scoringName}</strong><span>${selectedScoring === "match" ? "Every hole is won, lost, or tied." : "The lowest total strokes wins."}</span></div><div class="tb-round-tools">${["strokes", "match"].map(s => `<button class="btn ${selectedScoring === s ? "primary" : ""}" data-tb-scoring="${entry.round.id}:${s}" aria-pressed="${selectedScoring === s}">${s === "strokes" ? "Stroke play" : "Match play"}<small>${selectedScoring === s ? "Selected" : s === "match" ? "Hole by hole" : "Fewest strokes"}</small></button>`).join("")}</div>${matchupEditor(state, entry)}` : `<div class="tb-empty">Build the two-round schedule from Teams & Captains first.</div>`}</div></details>`;
  };
