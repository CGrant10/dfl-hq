...
function ensureStyles() {
  ...
  style.textContent += `body.route-light-content.tb-focus{background:#eef1f6}.route-light-content .tb-shell{background:linear-gradient(#f7f9fc,#eef1f6 70%);color:#11161d}.route-light-content .tb-head{background:#fff;border-bottom-color:#ccd4e0}.route-light-content .tb-circle{border-color:#ccd4e0;background:#eef1f6;color:#11161d}.route-light-content .tb-hole small,.route-light-content .tb-copy,.route-light-content .tb-match-title span,.route-light-content .tb-empty,.route-light-content .tb-member-hint{color:#5a6575}.route-light-content .tb-hole small .tb-yardage,.route-light-content .tb-sub small,.route-light-content .tb-link,.route-light-content .tb-title span,.route-light-content .tb-matchup-head,.route-light-content .tb-score-team strong,.route-light-content .tb-leader-head,.route-light-content .tb-leader-row em,.route-light-content .tb-quick-status,.route-light-content .tb-fold summary small,.route-light-content .tb-fold summary:after,.route-light-content .tb-roster-head strong,.route-light-content .tb-match-side>b{color:#003396}.route-light-content .tb-sub,.route-light-content .tb-scoreboard,.route-light-content .tb-leader,.route-light-content .tb-match-card,.route-light-content .tb-member-entry{background:#fff;border-color:#ccd4e0}.route-light-content .tb-sync{background:#f7f9fc;border-bottom-color:#e3e8f0;color:#0f7a3d}.route-light-content .tb-mode button,.route-light-content .tb-round-tabs button,.route-light-content .tb-team-actions .btn{border-color:#ccd4e0;background:#fff;color:#11161d}.route-light-content .tb-mode button.is-active,.route-light-content .tb-round-tabs button.is-active{border-color:#003396;background:#003396;color:#fff}.route-light-content .tb-leader-row,.route-light-content .tb-member-hint{border-color:#e3e8f0}.route-light-content .tb-leader-row small,.route-light-content .tb-score-side small,.route-light-content .tb-quick-status small,.route-light-content .tb-roster-head small,.route-light-content .tb-seat small{color:#5a6575}.route-light-content .tb-quick-add,.route-light-content .tb-score{border-color:#9aa8b7;background:#e6eaf1;color:#11161d}.route-light-content .tb-score b{color:#003396}.route-light-content .tb-leaderboard-field{background:linear-gradient(#f7f9fc,#eef1f6);color:#11161d}.route-light-content .tb-box,.route-light-content .tb-fold{border-color:#ccd4e0;background:#fff}.route-light-content .tb-fold[open] summary,.route-light-content .tb-roster-head,.route-light-content .tb-place{border-color:#e3e8f0}.route-light-content .tb-chip,.route-light-content .tb-ready span,.route-light-content .tb-unassigned{background:#eef1f6}.route-light-content .tb-add input,.route-light-content .tb-add select,.route-light-content .tb-select,.route-light-content .tb-team-name,.route-light-content .tb-seats select{border-color:#9aa8b7;background:#fff;color:#11161d}.route-light-content .tb-team-card,.route-light-content .tb-matchup{border-color:#ccd4e0;background:#f7f9fc}`;
...
function focusedMatchMarkup(state) {
  ...
  const matches = entry.battles.map(b => {
    ...
    return `<article class="tb-match-card"><div class="tb-match-title"><strong>Match ${b.match_number}</strong><span>${b.result ? esc(standingLine(b.result, names[0], names[1])) : "Waiting for both sides"}</span></div>${b.sides.map(s => {
      ...
      return `<div class="tb-score-side"><div><strong>${esc(sideName(s))}</strong><small>${esc(s.teamName)} · ${s.strokes.size ? `${total} strokes · Thru ${s.strokes.size}` : "Not started"}</small></div>${editable ? `<button class="tb-score" data-tb-open="${s.id}">Hole ${hole}<b>${scoreMark(row.strokes, parFor(state.holes, hole), "+")}</b></button>` : `<span class="tb-score is-readonly">Hole ${hole}<b>${scoreMark(row.strokes, parFor(state.holes, hole))}</b></span>`}</div>`;
    }).join("")}</article>`;
  }).join("");
  ...
  const memberEntry = `<section class="tb-member-entry"><div class="tb-member-hint">Your score entry · other players' scores are available under Scorecard</div>${mine.length ? mine.map(s => {
    ...
    return `<article class="tb-quick-player"><div class="tb-avatar">${esc(initials(sideName(s)))}</div><div><div class="tb-quick-name">${esc(sideName(s))}</div><div class="tb-quick-status">${s.strokes.size ? `${total} strokes` : `Hole ${hole}`} <small>· ${esc(s.teamName)} · Thru ${s.strokes.size}</small></div></div><button class="tb-quick-add" data-tb-open="${s.id}">${Number(row.strokes) ? `${scoreMark(row.strokes, parFor(state.holes, hole))}Edit score<small>Hole ${hole}</small>` : `<span>+</span>Add score<small>Hole ${hole}</small>`}</button></article>`;
  }).join("") : `<div class="tb-empty">You are not assigned to a ${entry.round.format === "pairs" ? "2v2" : "singles"} matchup yet.</div>`}</section>`;
  const table = `<table class="tb-table"><thead><tr><th class="sticky">Pair / golfer</th>${Array.from({ length: count }, (_, i) => i + 1).map(h => { const holeYards = yardageFor(state.courseHoles, h); return `<th>${h}<br><small>${holeYards ? `${holeYards} yd · ` : ""}Par ${parFor(state.holes, h)}</small></th>`; }).join("")}<th>Total</th></tr></thead><tbody>${sides.map(s => { const total = [...s.strokes.values()].reduce((a, n) => a + n, 0); return `<tr><th class="sticky">${esc(sideName(s))}<br><small>${esc(s.teamName)}</small></th>${Array.from({ length: count }, (_, i) => i + 1).map(h => `<td>${scoreMark(s.strokes.get(h), parFor(state.holes, h))}</td>`).join("")}<td>${total || "—"}</td></tr>`; }).join("")}</tbody></table>`;
  const gps = `<div class="tb-gps-slot" data-tb-gps-slot data-tb-hole-yardage="${yards}" data-tb-hole-par="${parFor(state.holes, hole)}"></div>`;
  const playBody = state.organizer ? `${scoreboard(state)}${tabs}${gps}${leaderboard}${matches}` : `${tabs}${gps}${memberEntry}`;
...
async function loadState(outing, organizer) {
  const id = outing.id, [tr, pr, hr, rr, mr, members, courseRes] = await Promise.all([db().from("golf_teams").select("*").eq("outing_id", id).order("sort_order"), db().from("golf_participants").select("*").eq("outing_id", id).order("sort_order"), db().from("golf_holes").select("hole,par").eq("outing_id", id).order("hole"), db().from("golf_rounds").select("*").eq("outing_id", id).order("round_number"), db().from("golf_matches").select("id,round_id,match_number").eq("outing_id", id).order("match_number"), loadMembers().catch(() => []), outing.course_id ? db().from("golf_course_holes").select("hole,par,yardage_men,yardage_women").eq("course_id", outing.course_id).order("hole") : { data: [], error: null }]);
  ...
  return { outing, organizer, teams, participants, holes, courseHoles, rounds, sides: builtSides, matchPlayers: mpr.data, scoreRows, members, names, individual: rounds.some(e => e.individual), stale: false };
}
