...
function calmSetupMarkup(state) {
  ...
  return `<main class="tb-shell" data-tbeta-root>
    ...
    <section class="tb-setup">
      <details class="tb-fold" data-tb-section="golfers" ${openSection === "golfers" || (!openSection && state.participants.length < 2) ? "open" : ""}><summary><span><small>STEP 1</small><strong>Golfers</strong></span><em>${state.participants.length} added</em></summary><div class="tb-fold-body"><div class="tb-add"><select data-tb-member><option value="">Choose a DFL golfer…</option>${available.map(m => `<option value="${m.id}">${esc(nameOf(m))}</option>`).join("")}</select><button class="btn" data-tb-add-member>Add</button></div><div class="tb-add"><input data-tb-guest placeholder="Guest golfer name"><button class="btn" data-tb-add-guest>Add guest</button></div><div class="tb-chips">${playerChips || "Add golfers to begin."}</div></div></details>
...
async function buildCustomMatch(state, sizes, holes, scoring) {
  if (state.teams.length !== 2) throw new Error("Create exactly two teams first.");
  ...
  const rosters = state.teams.map(team => state.participants.filter(person => String(person.team_id) === String(team.id)).sort((a, b) => Number(a.sort_order) - Number(b.sort_order)));
  const shortSide = rosters.findIndex((roster, index) => roster.length < sizes[index]);
  if (shortSide >= 0) throw new Error(`${state.teams[shortSide].name} needs at least ${sizes[shortSide]} golfer${sizes[shortSide] === 1 ? "" : "s"}.`);
  if (state.rounds.length && !confirm(`Replace the current Beta schedule with one ${sizes[0]}v${sizes[1]} custom match? Existing match scores will be removed.`)) return false;
