...
import { BETA_CUSTOM_MAX_SIDE, betaCaptainChoices, betaCustomSizes, betaFormatStatus, betaIsCustomRound, betaMatchCount, betaRoundLabel, betaRoundName, betaSeatsForSide } from "./golf-tournament-beta-format.js";
...
function ensureStyles() {
  ...
  style.textContent += `.tb-custom-grid{display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);align-items:end;gap:9px;margin-top:12px}.tb-custom-grid label{display:grid;gap:5px}.tb-custom-grid label span{overflow:hidden;color:var(--accent-2);font-size:9px;font-weight:950;text-overflow:ellipsis;white-space:nowrap;text-transform:uppercase}.tb-custom-grid>strong{padding-bottom:13px;color:var(--muted);font-size:9px}.tb-score{border-color:var(--control-line)!important;background:var(--bg-3)!important;color:var(--text)!important;box-shadow:var(--shadow)}.tb-score b{color:var(--accent-2)!important}`;
  style.textContent += `.tb-member-add{display:grid;gap:8px;margin-top:9px}.tb-member-picker{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;max-height:180px;overflow-y:auto;padding:7px;border:1px solid var(--line);border-radius:11px;background:var(--bg-3)}.tb-member-picker label{display:flex;align-items:center;gap:8px;min-width:0;min-height:38px;padding:6px 8px;border:1px solid var(--line-soft);border-radius:9px;background:var(--bg-2);font-size:11px;font-weight:800}.tb-member-picker input{width:18px;height:18px;flex:0 0 auto;accent-color:var(--accent-2-fill)}.tb-member-picker span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.tb-member-add>.btn{min-height:42px}@media(max-width:440px){.tb-member-picker{grid-template-columns:1fr}}`;
...
function calmSetupMarkup(state) {
  ...
  const customSetup = `<details class="tb-fold" data-tb-section="custom" ${openSection === "custom" || Boolean(status.custom) ? "open" : ""}><summary><span><small>MATCH OPTION</small><strong>Custom match</strong></span><em class="${status.customReady ? "is-ready" : ""}">${status.custom ? `${currentCustomSizes[0]} vs ${currentCustomSizes[1]} · ${status.customReady ? "Ready" : "Needs players"}` : "1–4 players per side"}</em></summary><div class="tb-fold-body"><p class="tb-copy">Build one uneven or even head-to-head match. Each side records one shared score per hole.</p><div class="tb-custom-grid"><label><span>${esc(state.teams[0]?.name || "Side 1")}</span><select class="tb-select" data-tb-custom-side="0">${sideSizeOptions(currentCustomSizes[0])}</select></label><strong>VS</strong><label><span>${esc(state.teams[1]?.name || "Side 2")}</span><select class="tb-select" data-tb-custom-side="1">${sideSizeOptions(currentCustomSizes[1])}</select></label></div><div class="tb-round-tools"><select class="tb-select" data-tb-custom-scoring><option value="strokes" ${scoringOf(status.custom?.round) === "strokes" ? "selected" : ""}>Stroke play</option><option value="match" ${scoringOf(status.custom?.round) === "match" ? "selected" : ""}>Match play</option></select><select class="tb-select" data-tb-custom-holes><option value="9" ${roundHoles(status.custom?.round) === 9 ? "selected" : ""}>9 holes</option><option value="18" ${roundHoles(status.custom?.round) === 18 ? "selected" : ""}>18 holes</option></select></div><div class="tb-actions"><button class="btn primary wide" data-tb-build-custom ${state.participants.length >= 2 ? "" : "disabled"}>${status.custom ? "Rebuild custom match" : "Build custom match"}</button></div>${status.custom ? matchupEditor(state, status.custom) : ""}</div></details>`;
  ...
  return `<main class="tb-shell" data-tbeta-root>
    ...
    <section class="tb-setup">
      <details class="tb-fold" data-tb-section="golfers" ${openSection === "golfers" || (!openSection && state.participants.length < 2) ? "open" : ""}><summary><span><small>STEP 1</small><strong>Golfers</strong></span><em>${state.participants.length} added</em></summary><div class="tb-fold-body">${memberPicker(available)}<div class="tb-add"><input data-tb-guest placeholder="Guest golfer name"><button class="btn" data-tb-add-guest>Add guest</button></div><div class="tb-chips">${playerChips || "Add golfers to begin."}</div></div></details>
      <details class="tb-fold" data-tb-section="teams" ${openSection === "teams" || (!openSection && (state.teams.length !== 2 || !state.rounds.length)) ? "open" : ""}><summary><span><small>STEP 2</small><strong>Teams & captains</strong></span><em class="${state.teams.length === 2 ? "is-ready" : ""}">${status.counts.join("–") || "0–0"} · Captains optional for custom</em></summary><div class="tb-fold-body"><p class="tb-copy">Name each team and place every golfer on a side. Captains are only required for the traditional two-round schedule.</p><div class="tb-team-actions"><button class="btn" data-tb-deal="even" ${state.teams.length !== 2 ? "disabled" : ""}>Place evenly</button><button class="btn" data-tb-deal="random" ${state.teams.length !== 2 ? "disabled" : ""}>Shuffle teams</button><button class="btn tb-reset" data-tb-create-teams>${state.teams.length === 2 ? "Reset teams" : "Create two teams"}</button></div>${teamControls}<div class="tb-actions"><button class="btn primary wide" data-tb-build-team-day ${status.teamsReady && status.captainsReady ? "" : "disabled"}>${status.pairs || status.singles ? "Reset both rounds" : "Build traditional matchups"}</button></div></div></details>
      ${customSetup}${guestAccess}${traditionalRounds}<div class="tb-status" data-tb-status></div>
    </section>
  </main>`;
}
...
async function buildCustomMatch(state, sizes, holes, scoring) {
  ...
  const roundId = Number(added.data), name = `Custom Match · ${sizes[0]}v${sizes[1]}`;
...
function wireSetup(root, state) {
  ...
  root.querySelector("[data-tb-build-custom]")?.addEventListener("click", e => busy(e.currentTarget, async () => {
    ...
    if (await buildCustomMatch(state, sizes, holes, scoring)) { setupSection.set(state.outing.id, "custom"); await paint(); }
