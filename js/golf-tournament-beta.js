...
const liveStat = (scores, cardId, holes, count) => {
  let strokes = 0, par = 0, thru = 0;
  for (let hole = 1; hole <= count; hole += 1) { const row = scores.get(`${cardId}:${hole}`); if (!Number(row?.strokes)) continue; strokes += Number(row.strokes); par += parFor(holes, hole); thru += 1; }
  return { strokes, thru, toPar: strokes - par };
};
const relative = value => value === 0 ? "E" : value > 0 ? `+${value}` : String(value);
const scoreMark = (strokes, par) => {
  const value = Number(strokes) || 0;
  if (!value) return "—";
  const delta = value - Number(par || 4), kind = delta <= -2 ? "eagle" : delta === -1 ? "birdie" : delta === 0 ? "par" : delta === 1 ? "bogey" : delta === 2 ? "double" : "triple";
  return `<span class="tb-mark ${kind}">${value}</span>`;
};
...
function markup(state) {
  ...
  const standings = cards.map(card => ({ card, ...liveStat(scores, card.id, holes, count) })).sort((a, b) => (b.thru > 0) - (a.thru > 0) || a.toPar - b.toPar || b.thru - a.thru || a.strokes - b.strokes);
  const leaderboard = `<section class="tb-live" aria-label="Live leaderboard"><div class="tb-live-head"><span>Live leaderboard</span><span>LIVE</span></div>${standings.slice(0, 4).map((entry, index) => `<div class="tb-live-row"><span>${index + 1}</span><strong>${esc(entry.card.primary)}</strong><small>${entry.thru ? `Thru ${entry.thru}` : "Not started"}</small><span class="tb-live-score">${entry.thru ? relative(entry.toPar) : "—"}</span></div>`).join("")}</section>`;
