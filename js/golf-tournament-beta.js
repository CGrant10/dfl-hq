...
function focusedMatchMarkup(state) {
  ...
  const memberLeaderboard = state.organizer ? "" : `<section class="tb-scorecard tb-leaderboard-view" data-tb-leaderboard-view hidden><header class="tb-card-head"><button class="tb-card-back" data-tb-leaderboard>‹ Back to scoring</button><h2>${entry.round.format.toUpperCase()} LEADERBOARD</h2></header><div class="tb-leaderboard-field">${scoreboard(state)}${leaderboard}</div></section>`;
...
function wireMatch(root, state) {
  ...
  root.querySelectorAll("[data-tb-leaderboard]").forEach(b => b.addEventListener("click", () => { const play = root.querySelector(".tb-play"), board = root.querySelector("[data-tb-leaderboard-view]"); if (!board) return; play.hidden = !play.hidden; board.hidden = !board.hidden; }));
