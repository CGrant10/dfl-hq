  ...
  const [leagues, standings, users, members, matchups, manual, arenaEvents, arenaResults, golf, config] =
    await Promise.all([
      db().from("sleeper_leagues")
          .select("season,status,champion_locked,last_place_user_id,last_place_locked,champion_user_id,runner_up_user_id,champion_roster_id,runner_up_roster_id,playoff_teams")
