...
function ownView(rows,me,isPublic){
  ...
  return `<section class="card bag-card">
    <div class="card-title-row"><div><div class="card-title">My bag</div><p class="muted tiny">${esc(me.display_name)} · ${isPublic?"visible to the league":"only you can see this"}</p></div><span class="bag-count">${rows.length}</span></div>
...
function boot(){const find=()=>{const el=document.querySelector("#golf-wrap .golf-bag-page");if(!el||el===host)return;host=el;wire();draw()};new MutationObserver(find).observe(document.body,{childList:true,subtree:true});find()}
