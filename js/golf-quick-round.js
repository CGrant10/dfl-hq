...
async function data(){
  const[c,m,r]=await Promise.all([
    ...
    loadMembers(),
    db().from("golf_quick_rounds").select("id,course_id,played_on,holes,status,golf_courses(name)").order("played_on",{ascending:false}).order("id",{ascending:false}).limit(6)
  ]);
  if(c.error||r.error)throw c.error||r.error;
  return{courses:c.data||[],members:m||[],recent:r.data||[]};
}
...
function paintHome(host,d){
  ...
  host.innerHTML=`<div class="gq-home"><div class="gq-head"><div><h2>Quick Round</h2><p class="muted tiny">Everyday golf. No teams, no tournament setup.</p></div><button class="btn small" data-gq-new>Start round</button></div><div data-gq-body>${d.recent.length?`<div class="gq-recent">${d.recent.map(r=>`<button data-gq-open="${r.id}"><span><strong>${esc(r.golf_courses?.name||"Round")}</strong><br><small class="muted">${esc(r.played_on)} · ${r.holes} holes</small></span><span>${r.status==="final"?"Final":"Open"}</span></button>`).join("")}</div>`:`<p class="muted tiny">No quick rounds yet.</p>`}</div></div>`;
...
async function start(host,d){
  const body=host.querySelector("[data-gq-body]"),course=Number(body.querySelector("[data-gq-course]").value),holes=Number(body.querySelector("[data-gq-holes]").value),ids=[...body.querySelectorAll("[data-gq-member]:checked")].map(x=>Number(x.value)),guest=body.querySelector("[data-gq-guest]").value.trim();
  ...
  try{
    const{data:r,error}=await db().from("golf_quick_rounds").insert({course_id:course,created_by:Number(getMemberId()),holes}).select().single();
...
function paintRound(host,r,players,scores,holeRows,d){
  ...
  host.innerHTML=`<div class="gq-play" data-gqm-entry data-active-hole="${hole}" data-quick-player-card data-quick-active="true">
    ...
    <nav class="gq-bottom" aria-label="Quick round tools"><button class="is-active" type="button"><b>▦</b>Scorecard</button><button type="button" data-gq-notes><b>✎</b>Notes</button><button type="button" data-gq-camera><b>◉</b>Camera</button><button type="button" data-gq-games><b>♢</b>Games</button><button type="button" data-gq-gps><b>◎</b>GPS ON</button></nav>
  </div>`;
  ...
  host.querySelector("[data-gq-notes]").onclick=()=>toast("Quick notes are next up");host.querySelector("[data-gq-camera]").onclick=()=>toast("Camera shortcut coming next");host.querySelector("[data-gq-games]").onclick=()=>toast("Quick games coming next");
...
function showScoreSheet(host,r,players,scores,holeRows,d,playerId,hole,startValue){
  ...
  const sheet=document.createElement("div");sheet.className="gq-score-sheet";sheet.innerHTML=`<div class="gq-score-card"><h3>${esc(name)} · Hole ${hole}</h3><p>Enter strokes for this hole.</p><div class="gq-stepper"><button type="button" data-minus>−</button><input data-score type="number" inputmode="numeric" min="1" max="15" value="${Math.max(1,Math.min(15,Number(startValue)||4))}"><button type="button" data-plus>+</button></div><div class="gq-sheet-actions"><button class="btn" type="button" data-cancel>Cancel</button><button class="btn primary" type="button" data-save>Save score</button></div></div>`;document.body.appendChild(sheet);
  const input=sheet.querySelector("[data-score]");
  const bump=n=>input.value=String(Math.max(1,Math.min(15,(Number(input.value)||1)+n)));
  sheet.querySelector("[data-minus]").onclick=()=>bump(-1);sheet.querySelector("[data-plus]").onclick=()=>bump(1);sheet.querySelector("[data-cancel]").onclick=()=>sheet.remove();sheet.onclick=e=>{if(e.target===sheet)sheet.remove()};
  sheet.querySelector("[data-save]").onclick=async()=>{const strokes=Number(input.value);if(strokes<1||strokes>15)return toast("Score must be 1–15",true);const q=await db().from("golf_quick_scores").upsert({player_id:playerId,hole,strokes},{onConflict:"player_id,hole"});if(q.error)return toast(q.error.message||"Score did not save",true);sheet.remove();await openRound(host,r.id,d)};
}
