...
begin
  ...
  if length(coalesce(new_pin, '')) < 4 then raise exception 'Commissioner PIN must be at least 4 characters'; end if;
  ...
  values (target_member_id, crypt(new_pin, gen_salt('bf')), coalesce(new_permissions, '[]'::jsonb), make_owner, true, now())
