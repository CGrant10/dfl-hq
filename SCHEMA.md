...
### The Sportsbook sells entries, not just singles

| # | File | What it establishes |
|---|---|---|
| 46 | `sportsbook_entries_schema.sql` | `sportsbook_bet_legs`, `sportsbook_bets.pick_count` / `.cancelled_at` / `.dismissed_at`, `sportsbook_place_entry()`, `sportsbook_cancel_bet()`, `sportsbook_dismiss_bet()`, `sportsbook_roll_up_entry()`, the pricing helpers (`_decimal_odds`, `_combine_odds`, `_odds_payout`), and **replaces** `sportsbook_place_bet()`, `sportsbook_settle_market()` and `sportsbook_void_market()`. Needs 33, and must run **after** 34 and 40 or their older bodies win. **`sportsbook_bets` becomes the entry**, rather than entries getting a table of their own beside it — the row already held the stake, the price, the payout and the status, so the alternative meant two settle paths, two lists in the UI and two definitions of "open". The picks move to legs and every existing single is backfilled as a one-leg entry, so a one-pick entry prices and settles exactly as a straight bet always did. `market_id` / `outcome_id` stay on the entry and stay populated for one-pick entries, which is the back-compatible read path; they are null on a multi-pick, which is why the board gathers what a member holds from the **legs** and not from those columns. A unique index on `(bet_id, market_id)` refuses both sides of one game in one entry. **Settling grades legs, then rolls the entry up** — lost the moment any leg loses, won only when all of them have. **Voiding a market refunds the whole entry** rather than dropping the dead leg and re-pricing the rest: re-pricing means telling somebody their 4-pick at +2200 is now a 3-pick at +900 after they agreed a price, and the house here is a group chat. Cancelling is refused once any leg's market has locked, because by then it is a free look at a result. Dismissing moves no SIN — the row and its receipts stay. |
| 47 | `sportsbook_trends_schema.sql` | `sportsbook_trending_picks()` — aggregate ticket and bettor counts for the three most-backed picks on current Fantasy lines. It exposes no member identities or stakes. Needs 46. |

### Anonymous real-user performance
...
| 50 | `custom_breaking_alerts_schema.sql` | Commissioner-authored breaking banners with a short label, headline, supporting text and safe in-app destination. Needs 31 and `notifications_schema.sql`. Members can read active alerts; only a commissioner with Sleeper permission can launch or end one. Optional phone delivery uses the existing Announcements preference. |
...
| `custom_breaking_alerts_schema.sql` | Automatic trade coverage still works. The custom composer explains which SQL file to run, and the shared banner quietly ignores custom alerts until it exists. |
